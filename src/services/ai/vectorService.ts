...
export const vectorService = {
    ...
    /**
     * Generates embedding for a given text using ONLY gemini-embedding-001
     */
    async getEmbedding(text: string): Promise<number[] | null> {
        if (!text || text.trim().length === 0) return null;

        try {
            // STRICTLY use gemini-embedding-001 as requested.
            // No fallbacks to text-embedding-004 or embedding-001.
            ...
            console.error("[VectorService] Error generating embedding with gemini-embedding-001:", error);
            return null;
        }
    },
